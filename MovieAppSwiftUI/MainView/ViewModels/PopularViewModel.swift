...
class PopularViewModel: SegmentViewModelProtocol {
    ...
    func getMovies() {
        ...
        isLoading = true
        MovieManager.shared.getPopular(page: 1) { [weak self] response in
    ...
    func loadNextPage() {
        isLoading = true
        MovieManager.shared.getPopular(page: page + 1) { [weak self] response in
            guard let self else { return }
            self.isLoading = false
            switch response {
            case .success(let model):
